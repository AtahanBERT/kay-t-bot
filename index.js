...
client.permission = async function (userID, type) {
  ...
  const x = {
    ...
    Founder: [],
    Ceo: [],
    Register: ["1016275796190568498"],
  }

  let staffPerms = member.hasPermission('ADMINISTRATOR') || config.Owners.includes(member.id) || member.id == config.OwnerID;
...
client.login(process.env.token).then(() => client.logger.log("Client Successfully Launched. ✔", "ready")).catch(() => client.logger.log("Client Connection Failed. ✘", "error"))