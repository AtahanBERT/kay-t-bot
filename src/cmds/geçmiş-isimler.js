    ...
    execute: async (client, message, args) => {
        ...
        if(client.permission(message.author.id, "register")) {
            ...
            if(!data) {
                ...
                return client.send(`Kullanıcının veritabanında hiç bir kayıt işlemi bulunamadı. ${settings.emojis.iptal}`, message.author, message.channel)
            ...
            Collector.on("collect", async (button) => {
                ...
                if(button.id === "names") {
                    ...
                    .setAuthor(message.guild.name, message.guild.iconURL({dynamic: true}))
                    .setDescription(`Kullanıcının **${data.names.length}** tane ismi veritabanında kayıtlı, dökümanlar aşağıya listelendi. ${settings.emojis.iptal}\n\n${data.names.map(x => `\`• ${x.displayName}\` (${x.gender}) `).join('\n')}`)
                    .setColor('RANDOM')
                    .setFooter(config.Footer)
